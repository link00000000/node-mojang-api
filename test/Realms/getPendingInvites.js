...
test("Realms/getPendingInvites", async t => {
    ...
    if(process.env.OFFLINE)
    {
        ...
        nock(REALMS_SERVER)
            ...
            .get("/invites/count/pending").reply((uri, req) => {
                return 0;
            });
    }

    let auth = new Auth(process.env.EMAIL, process.env.PASSWORD, path.join(__dirname, "getPendingInvites.js.client.json"));
    await auth.authenticate();
    let realms = new Realms(auth);
    let worlds = await realms.getWorlds();
    let numInvites = await realms.getPendingInvites(worlds[0]);
    t.pass();
