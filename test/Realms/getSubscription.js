...
test("Realms/getSubscription", async t => {
    ...
    if(process.env.OFFLINE)
    {
        ...
        nock(REALMS_SERVER)
            ...
            .get(/\/subscriptions\/\d*/).reply((uri, req) => {
                return {
                    startDate: 0,
                    daysLeft: -1,
    ...
    let auth = new Auth(process.env.EMAIL, process.env.PASSWORD, path.join(__dirname, "getSubscription.js.client.json"));
    await auth.authenticate();
    let realms = new Realms(auth);
    let worlds = await realms.getWorlds();
    let subscription = await realms.getSubscription(worlds[0]);
    t.pass();
